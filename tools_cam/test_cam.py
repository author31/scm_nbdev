...
from scm_nbdev.config.default import update_config
from scm_nbdev.config.default import config as cfg
from scm_nbdev.core.engine import create_data_loader
...
from scm_nbdev.cams_deit import resize_cam, blend_cam, get_bboxes, cal_iou, draw_bbox
...
def main():
    ...
    device, model, cls_criterion = create_model(cfg, args)
    ...
    with torch.no_grad():
        ...
        for _, (input, gt_labels, bbox, image_names, orig) in tqdm(enumerate(dataloader)):
            ...
            for i in range(len(inputs)):
                ...
                save_dir = os.path.join(cfg.BASIC.SAVE_DIR, 'preds', 'boxed_image')
                ...
                save_dir = os.path.join(cfg.BASIC.SAVE_DIR, 'preds', 'roi_images')
                save_path = os.path.join(save_dir, image_name)
                mkdir(save_dir)
                cv2.imwrite(save_path, roi_image)
                
                save_dir = os.path.join(cfg.BASIC.SAVE_DIR, 'preds', 'binary')
                save_path = os.path.join(save_dir, image_name)
                mkdir(save_dir)
...
def evaluate(
    ...
    Args:
        ...
        metric_options (dict | None): Options for calculating metrics.
            ...
            Default: None.
        logger (logging.Logger | None): Logger used for logger.infoing
